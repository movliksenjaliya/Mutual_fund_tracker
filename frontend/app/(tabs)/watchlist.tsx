...
export default function Watchlist() {
  ...
  const saveTarget = async () => {
    ...
    await api.updateWatchlist(targetModal.id, {
      target_buy_price: isNaN(v) ? null : v,
    });
    setTargetModal(null);
    setTargetInput("");
    load();
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          style={styles.addBtn}
          onPress={() => router.push("/search?mode=watchlist")}
        >
          ...
          return (
            ...
            >
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" }}>
                ...
                <TouchableOpacity
                  testID={`watchlist-remove-${item.scheme_code}`}
                  onPress={() => remove(item.id, item.scheme_name)}
                  hitSlop={10}
                >
...
const styles = StyleSheet.create({
  ...
  modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "center", padding: spacing.p6 },

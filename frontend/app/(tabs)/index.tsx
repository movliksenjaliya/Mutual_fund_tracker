...
export default function Dashboard() {
  ...
  const heroBg = portfolioPnl >= 0 ? colors.brand : (colors === undefined ? "#5C2A24" : colors.negative);
  // Use brand color for profit, deep red for loss — works in both themes
  const heroBgFinal = portfolioPnl >= 0 ? colors.brand : "#7A3530";
  const heroTextColor = "#FFFFFF";
  const heroSubtleColor = portfolioPnl >= 0 ? colors.brandSoft : "#D1A8A4";
    ...
    >
      <View style={styles.header}>
        <View>
          ...
          <Text style={[typography.h1, { marginTop: 4, color: colors.textPrimary }]} testID="dashboard-title">
            Your portfolio
          </Text>
        </View>
      ...
      <TouchableOpacity
        ...
        style={[styles.heroCard, { backgroundColor: heroBgFinal }]}
      >
        <View style={styles.heroHeader}>
          <Text style={[typography.overline, { color: heroSubtleColor }]}>TOTAL VALUE</Text>
          <Feather name="chevron-right" size={18} color={heroSubtleColor} />
        </View>
        <Text style={[typography.financialLarge, { color: heroTextColor, marginTop: 4 }]} testID="hero-current-value">
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: spacing.p2, marginTop: spacing.p2 }}>
          <View style={[styles.heroPill, { backgroundColor: "rgba(255,255,255,0.18)" }]}>
            <Text style={[typography.bodySmall, { fontWeight: "700", color: heroTextColor }]}>
              {portfolioPnl >= 0 ? "▲" : "▼"} {Math.abs(portfolioPnlPct).toFixed(2)}%
            </Text>
          </View>
          <Text style={[typography.bodySmall, { color: heroTextColor }]}>
            {portfolioPnl >= 0 ? "+" : ""}
        ...
        <View style={styles.heroFooter}>
          <View style={{ flex: 1 }}>
            <Text style={[typography.bodySmall, { color: heroSubtleColor }]}>Invested</Text>
            <Text style={[typography.h4, { color: heroTextColor, marginTop: 2 }]}>{formatINR(portfolio?.summary?.total_invested)}</Text>
          </View>
          <View style={[styles.heroFooterCol, { flex: 1 }]}>
            <Text style={[typography.bodySmall, { color: heroSubtleColor }]}>Holdings</Text>
            <Text style={[typography.h4, { color: heroTextColor, marginTop: 2 }]}>{portfolio?.items?.length || 0}</Text>
          </View>
        </View>
      </TouchableOpacity>
        ...
        >
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-end" }}>
            <View>
              ...
              <Text style={[typography.bodySmall, { marginTop: 2, color: colors.textSecondary }]}>
          ...
          {niftyHistory.length > 1 && (
            <View style={{ marginTop: spacing.p3, marginHorizontal: -spacing.p4 }}>
              <LineChart data={niftyHistory} width={chartWidth} height={50} stroke={colors.brand} />
            </View>
          )}
        </TouchableOpacity>
      </View>
